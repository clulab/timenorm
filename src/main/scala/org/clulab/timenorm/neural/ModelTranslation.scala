...
class ReverseLambdaDim1 extends SameDiffLambdaLayer {
  override def defineLayer(sd: SameDiff, x: SDVariable): SDVariable = sd.reverse( "reversed", x, Array(0, 1):_*)
  override def getOutputType(layerIndex: Int, inputType: InputType): InputType = inputType
}

class ReverseLambdaDim2 extends SameDiffLambdaLayer {
...
object ModelTranslation {
  ...
  def main(args: Array[String]): Unit = {
    ...
    KerasLayer.registerLambdaLayer("lambda_2", new ReverseLambdaDim2())
