...
class TemporalNeuralParserTest extends FunSuite {
  ...
  private val anchor = parser.dct(parser.parse(List("2018-07-06")).head)
  private val test1 = "2018-10-10"
  private val test2 = "January"
  private val test3 = "Friday."
  ...
  private val test5 = """A substantial decline in gas revenue since 2014 has
                |contributed to a sharp drop in both foreign currency
                |reserves and the value of the South Sudanese pound.""".stripMargin
  private val dates = List(test1, test2, test3, test4, test5)
  ...
  test("last") {
    assert(intervals(2).head._1 == (0, 6))
  ...
  test("since-year") {
    assert(intervals(4).head._1 == (37, 47))
    assert(intervals(4).head._2.head._1 === LocalDateTime.of(2014, 1, 1, 0, 0))
    assert(intervals(4).head._2.head._2 === LocalDateTime.of(2018, 7, 7, 0, 0))
    assert(intervals(4).head._2.head._3 === 142387200)
  }
}
